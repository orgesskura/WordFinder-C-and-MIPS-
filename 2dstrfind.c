...
int rows=0;// nr of words in the grid without null character
...
{
  while (1) {
    if(*string=='\n'){                           // if i reached end of row check if it is end of word in dictionary
      return (*word == '\n');
    }
    if (*string != *word){
      return (*word == '\n');
    ...
    string++;                                // get nxt letter in grid
    word++;                                 // get next letter in dictionary
  }
...
int containV(char *string,char *word,int row,int idx){
  ...
  while (1) {
    ...
    if(idx+(rows/nr_rows)>=(rows/nr_rows)*(nr_rows)){       // if i am i nthe last row then check the if the next letter in word is newline
...
int containD(char *string, char *word,int row,int idx,int x1)
{
  while (1) {
   if (*string != *word) {
    ...
    if(idx+(rows/nr_rows)+1<(rows/nr_rows)*(nr_rows)){            // check if i am in the last row
     if(x1==(rows/nr_rows)-2){
                                                                 // if i am on the last column check if it is end of word in dictionary      
        word++; 
       return (*word=='\n');  
     }
    else{
    string+=(rows/nr_rows)+1;
       idx+=(rows/nr_rows)+1;
       word++;                                          // if i am not go one row below and update row and grid_idx
       row++;
       x1++;
    }}
    else {
       
                                                      // if it is last column check if it is also end of word in dictionary
       word++;
       return (*word=='\n');
    }
